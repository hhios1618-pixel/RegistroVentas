...
type Payload = {
  person_id: string; // DEBE ser el UUID de public.people
  site_id: string;
  type: CheckType;
  device_id?: string;
  qr_code?: string;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (pErr) {
      console.error("[checkin] person lookup failed:", pErr);
      return new Response(JSON.stringify({ error: "person_lookup_failed", details: pErr.message }), {
        status: 500, headers: cors,
      });
    }
    if (!person) {
    ...
    if (person.active === false) {
      return new Response(JSON.stringify({ error: "person_inactive" }), {
    ...
    const { data: site, error: sErr } = await supabase
      .from("sites")
      .select("id, lat, lng, radius_m, is_active, name")
      .eq("id", body.site_id)
      .single();
      ...
      if (site.lat != null && site.lng != null) {
        ...
        if (distance_m > radius && !withinAccuracyMargin) {
          return new Response(JSON.stringify({
              ...
              distance_m, required_radius: radius, accuracy: body.accuracy
            }), { status:403, headers: cors }
          );
        }
      }
    ...
    // ---------- INSERT ----------
    const isLunch = body.type === "lunch_in" || body.type === "lunch_out";
    const row = {
      person_id: effectivePersonId,
      site_id: body.site_id,
      type: body.type,
      lat: isLunch ? 0 : body.lat!,
      lng: isLunch ? 0 : body.lng!,
      selfie_path: isLunch ? "" : (selfiePath || ""),
