...
type Payload = {
  ...
  accuracy: number;
  device_id: string;
  selfie_base64: string; // dataURL o base64 puro
  qr_code: string;
};
  ...
  try {
    ...
    for (const k of required) {
      ...
      if ((body as any)[k] === undefined || (body as any)[k] === null) {
        return new Response(
          JSON.stringify({ error: "missing_field", field: k }),
          { status: 400, headers: { "content-type": "application/json", ...corsHeaders } },
        );
      }
    }
    if (body.type !== "in" && body.type !== "out") {
      return new Response(
        JSON.stringify({ error: "type_invalid" }),
        { status: 400, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
    // Precisión exigida (web móvil con muestreo)
    if (typeof body.accuracy === "number" && body.accuracy > 50) {
      return new Response(
        JSON.stringify({ error: "accuracy_too_high", accuracy: body.accuracy }),
        { status: 400, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }

    // Persona (no promotores, solo activos)
    ...
    if (pErr || !person) {
      return new Response(
        JSON.stringify({ error: "person_not_found" }),
        { status: 404, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
    if (person.active === false) {
      return new Response(
        JSON.stringify({ error: "person_inactive" }),
    ...
    const { data: site, error: sErr } = await supabase
      .from("sites")
      .select("id, lat, lng, radius_m, is_active")
      .eq("id", body.site_id)
      .single();
    if (sErr || !site) {
      return new Response(
    ...
    if (site.is_active === false || site.lat == null || site.lng == null) {
      return new Response(
        JSON.stringify({ error: "site_inactive_or_unset" }),
        { status: 400, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
    ...
    if (qErr || !qr) {
      return new Response(
        JSON.stringify({ error: "qr_invalid_or_expired" }),
        { status: 403, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
    ...
    const selfiePath =
      `${body.site_id}/${body.person_id}/${Date.now()}.jpg`;
    ...
    if (upErr) {
      return new Response(
        JSON.stringify({ error: "upload_failed", details: upErr.message }),
        { status: 500, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
    ...
    if (insErr) {
      return new Response(
        JSON.stringify({ error: "insert_failed", details: insErr.message }),
        { status: 500, headers: { "content-type": "application/json", ...corsHeaders } },
      );
    }
