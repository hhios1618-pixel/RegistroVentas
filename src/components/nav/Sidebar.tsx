  ...
  return (
    <aside
      ...
      className={[
        ...
        'bg-slate-950/95 backdrop-blur-sm border-r border-slate-800/60',
    ...
    >
      ...
      <div className="p-6 border-b border-slate-800/60">
        <Link href={ROUTES.DASH} className="flex items-center gap-3 group">
          <div className="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0">
            <Image src="/1.png" alt="Fenix Store" width={32} height={32} className="w-full h-full object-contain" />
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-white text-lg tracking-tight group-hover:text-emerald-400 transition-colors">
              Fenix Store
            </div>
            <div className="text-xs text-slate-500 -mt-0.5">Sistema de Gestión</div>
          </div>
        </Link>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-8 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        <div>
          <NavLink
            item={{ href: ROUTES.DASH, icon: <IconDashboard className="w-4 h-4" />, label: 'Dashboard', shortcut: 'H' }}
        ...
        {SECTIONS.map((section) => {
          ...
          return (
            <div key={section.title} className="space-y-4">
              <h3 className="px-3 text-xs font-medium text-slate-500 uppercase tracking-wider">{section.title}</h3>
      ...
      <div className="p-4 border-t border-slate-800/60">
        <div className="flex items-center gap-3 mb-4 p-2 rounded-lg hover:bg-slate-900/50 transition-colors">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-sm font-medium text-slate-200 truncate">{userName || 'Usuario'}</div>
