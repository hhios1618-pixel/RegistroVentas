    ...
    <div className="min-h-screen">
          ...
          <div className="relative backdrop-blur-xl bg-slate-950/80 border-b border-white/10">
            <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl font-extrabold text-white">Hola, {name}</h1>
                  <p className="text-white/60 text-sm">Esto es lo que importa hoy</p>
                </div>
              </div>
              <Link
                href="/dashboard/sales-report"
                className="px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-sm text-white hover:bg-white/15"
              >
                Ver reportes →
              </Link>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-white truncate">{it.titulo}</div>
          ...
          <Panel>
            <h3 className="text-lg font-bold text-white mb-3">Mi actividad</h3>
            <div className="grid gap-2">
...
function Panel({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  // ✅ CAMBIO: Usamos la clase .card de tu globals.css para consistencia total.
  return (
...
function KpiCard({ title, value, hint }: { title: string; value: string; hint?: string }) {
  // ✅ CAMBIO: Usamos la clase .card de tu globals.css. Se quita el motion.div exterior
  // porque el efecto de hover ya debería manejarse globalmente o puede añadirse después.
  // La animación de entrada se puede lograr con una clase.
  return (
    <div className="card apple-fade-in">
      <div className="text-sm apple-muted">{title}</div>
...
function QuickAction({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link
      href={href}
      className="group flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 text-white"
    >
      {icon}
