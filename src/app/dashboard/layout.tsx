// src/app/dashboard/promotores/layout.tsx
'use client';
...
({ link, isActive = false }) => (
  <Link
    href={link.href}
    className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors group relative
...
const Sidebar: FC<{ userRole: Role; userName: string }> = ({ userRole, userName }) => {
  ...
  return (
    <aside className="w-72 fixed top-0 left-0 h-screen bg-[#0D1117]/95 backdrop-blur border-r border-[#30363D] z-30 hidden lg:flex flex-col">
      {/* Brand */}
      <div className="p-4 border-b border-[#30363D]">
        <Link href="/" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-emerald-500 flex items-center justify-center font-extrabold text-black shadow-md">F</div>
        ...
        {sections.map((sec) => {
          ...
          return (
            <div key={sec.title} className="pt-1">
              <h3 className="px-3 text-[11px] font-semibold text-[#8B949E] uppercase tracking-widest mb-2">{sec.title}</h3>
              <div className="space-y-1">{links.map((l) => <NavLink key={l.href} link={l} />)}</div>
            </div>
          );
        })}
      </nav>
