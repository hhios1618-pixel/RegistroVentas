...
export async function GET(req: NextRequest) {
  try {
    // Leer la cookie de sesión directamente del request
    ...
    // Crear cliente admin para consultar la DB
    const admin = createClient(URL, SERVICE, { auth: { persistSession: false } });

    // Buscar la persona en la DB usando el ID del JWT
    const { data: person, error } = await admin
      .from('people')
      .select('id, full_name, role, fenix_role, privilege_level, active, email')
      .eq('id', payload.sub)
      .maybeSingle();
    ...
    // Usar fenix_role si existe, sino role
    const rawRole = person.fenix_role || person.role;
    const role = normalizeRole(rawRole);

    console.log('Respuesta exitosa para usuario:', {
      id: person.id,
      full_name: person.full_name,
      role,
      privilege_level: person.privilege_level,
      email: person.email,
      raw_role: rawRole
    });
