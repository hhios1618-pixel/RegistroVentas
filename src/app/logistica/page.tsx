// src/app/logistica/page.tsx (VERSIÓN FINAL CON NÚMEROS DE PEDIDO CORRECTOS)
'use client';

import React, { useEffect, useState, useCallback, useMemo } from 'react';
import { createClient } from '@supabase/supabase-js';
// ▼▼▼ CORRECCIÓN: Se añade 'EnrichedDeliveryRoute' para el nuevo tipo de dato ▼▼▼
import type { OrderRow, DeliveryUser, OrderStatus, DeliveryRoute, DeliveryMetrics, DeliveryStats, EnrichedDeliveryRoute } from '@/types';
...
// --- Iconos ---
import { 
  PlusCircle, RefreshCw, Filter, Search, Truck, 
  Package, Clock, CheckCircle2, AlertTriangle, Users
} from 'lucide-react';
...
export default function LogisticaPage() {
  ...
  // ▼▼▼ CORRECCIÓN: El estado ahora guarda las rutas enriquecidas ▼▼▼
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // ▼▼▼ CORRECCIÓN: "Enriquecemos" las rutas con el order_no ▼▼▼
      const enrichedRoutes = routesData.map(route => {
        const order = ordersData.find(o => o.id === route.order_id);
        return {
          ...route,
          order_no: order?.order_no || null // Añadimos el número de pedido
        };
      });

      setOrders(ordersData);
      setDeliveries(deliveriesRes.data as DeliveryUser[]);
      setDeliveryRoutes(enrichedRoutes); // Guardamos las rutas enriquecidas
  ...
  if (error) { return <div className="min-h-screen bg-gray-900 flex items-center justify-center text-red-400"><AlertTriangle className="mr-4"/>Error: {error}</div>; }
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-screen-2xl mx-auto">
          ...
          <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="small" onClick={loadData}><RefreshCw className="w-4 h-4 mr-2" />Actualizar</Button>
              <Button size="small" className="bg-blue-600 hover:bg-blue-700"><PlusCircle className="w-4 h-4 mr-2" />Nuevo Pedido</Button>
            </div>
          </header>
            ...
            <aside className="lg:col-span-1 space-y-6 sticky top-24">
              ...
              <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
                {activeDeliveries.length > 0 ? activeDeliveries.map(delivery => {
                  ...
                  const metrics = deliveryMetrics.find(m => m.delivery_user_id === delivery.id && m.metric_date === today);
                  ...
                  return <DeliveryCard key={delivery.id} delivery={delivery} stats={deliveryStats} onViewDetails={() => setSelectedDelivery(delivery)} />;
                }) : (
              ...
              <Card className="bg-gray-800/50 border-gray-700">
                <CardHeader>
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <h3 className="text-white font-bold text-lg">Lista de Pedidos ({filteredOrders.length})</h3>
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <div className="relative flex-grow">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input type="text" placeholder="Buscar..." value={filters.search} onChange={e => setFilters(p => ({...p, search: e.target.value}))} className="w-full bg-gray-900/50 border border-gray-600 rounded-md pl-9 pr-3 py-2 text-sm"/>
                      </div>
                    </div>
                  </div>
                </CardHeader>
